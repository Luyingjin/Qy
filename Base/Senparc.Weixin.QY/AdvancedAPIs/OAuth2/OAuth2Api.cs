...
namespace Senparc.Weixin.QY.AdvancedAPIs
{
    ...
    public static class OAuth2Api
    {
        ...
        public static string GetCode(string corpId, string redirectUrl, string state, string agentId, string responseType = "code", string scope = "snsapi_base")
        {
            ...
            {
                var url = string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type={2}&scope={3}&state={4}&agentid={5}#wechat_redirect", corpId.AsUrlData(), redirectUrl.AsUrlData(), responseType.AsUrlData(), scope.AsUrlData(), state.AsUrlData(), agentId.AsUrlData());
        ...
        /// 权限说明：管理员须拥有agent的使用权限；agentid必须和跳转链接时所在的企业应用ID相同。
        /// <returns></returns>
        public static GetUserDetailResult GetUserDetail(string accessToken, string user_ticket)
        {
            var url = string.Format("https://qyapi.weixin.qq.com/cgi-bin/user/getuserdetail?access_token={0}", accessToken.AsUrlData());

            var userDictionary = new Dictionary<string, string>();
            userDictionary["user_ticket"] = user_ticket;
            return Post.PostFileGetJson<GetUserDetailResult>(url, null, null, userDictionary, null);
        }
        #endregion
        ...
        /// 权限说明：管理员须拥有agent的使用权限；agentid必须和跳转链接时所在的企业应用ID相同。
        /// <returns></returns>
        public static async Task<GetUserDetailResult> GetUserDetailAsync(string accessToken, string user_ticket)
        {
            var url = string.Format("https://qyapi.weixin.qq.com/cgi-bin/user/getuserdetail?access_token={0}", accessToken.AsUrlData());

            var userDictionary = new Dictionary<string, string>();
            userDictionary["user_ticket"] = user_ticket;
            return await Post.PostGetJsonAsync<GetUserDetailResult>(url, null, userDictionary, null);
        }
        #endregion
    }
}
